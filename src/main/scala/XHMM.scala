...
object XHMM {
  private def forwardE(t: Target, j: State): BigDecimal = {
    val key = (t, j)
    if (fwdCache contains key) {
      fwdCache(key)
    } else {
      val result: BigDecimal  = if (t > 0) {
        (states map { i =>
          forwardE(t - 1, i) * transitions(t, i, j)
        } sum) * emissions(j, t)
      } else {
        start(j) * emissions(j, 0)
      }
      fwdCache(key) = result
      result
    }
  ...
  private def backwardE(t: Target, i: State): BigDecimal = {
    val key = (t, i)
    if (bckCache contains key) {
      bckCache(key)
    } else {
      // This does not currently support t = 0
      val result: BigDecimal = if (t < obs.length) {
        states map { j =>
          transitions(t, i, j) * emissions(j, t) * backwardE(t + 1, j)
        } sum
      } else {
        BigDecimal(1.0)
      }
  ...
  private def gamma(t: Target): BigDecimal = {
    states map { s => forwardE(t, s) * backwardE(t+1, s)} sum
  }
  ...
  def viterbi(observations: List[Double],
    ...
    def mostLikelyPath() : List[String] = {
      ...
      for (t <- 0 to 264) {
        path += (states map { (s) => (forwardE(t, s) * backwardE(t+1, s), s)
        } maxBy (_._1))._2
      }
      path.toList
    }
